        ...
        const result = await callDeepseekOcr({
          ...
          onProgress: ({ value, stage }) => {
            if (typeof value === 'number') {
              setOcrProgress(Math.max(0, Math.min(100, value)))
            }
            if (stage) {
              setOcrStage(stage)
            }
    ...
    <div className={styles.landing}>
      ...
      >
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          ...
          <p style={{ marginTop: 20, marginBottom: 10, fontSize: 16 }}>
            {selectedOCRCommand?.label}
          </p>
          <Progress
            percent={ocrProgress}
            status={ocrProgress === 100 ? 'success' : 'active'}
            strokeColor={{
              from: '#108ee9',
              to: '#87d068',
            }}
